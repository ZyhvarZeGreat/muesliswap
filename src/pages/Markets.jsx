import React from "react";
import "./Markets.css";
import img from "../assets/ada.webp";
const Markets = () => {
  return (
    <div id="root">
      <div className="Toastify"></div>

      <div height="56" className="sc-gEvEer kvHQza"></div>
      <main className="sc-kOPcWz hierkx">
        <div className="sc-iHGNWf hQvJpi">
          <h2
            style={{
              fontFamily: "gilroy",
              fontWeight: 600,
              fontSize: "40px",
              letterSpacing: "-0.64px",
            }}
            color="text"
            className="sc-gEvEer fgSxLa"
          >
            <span color="main" className="sc-gEvEer hRDVRe">
              Cardano
            </span>{" "}
            Featured Markets
          </h2>
          <div className="sc-ibQAlb fZnOPv">
            <div className="sc-krNlru iDYjIk">
              <div className="sc-jaXxmE eizVwN">
                <h2 className="sc-gEvEer fUmKfJ">🔥 Top Volume</h2>
                <a
                  aria-label="Show 🔥 Top Volume"
                  className="sc-hwdzOV jzRhqN"
                  href="/markets?sortBy=volume1d&amp;sortDir=true"
                >
                  <svg
                    height="12"
                    viewBox="0 0 7 12"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M.3 11.7a1 1 0 0 1 0-1.4l4.05-4.06a.33.33 0 0 0 0-.48L.29 1.71A1 1 0 1 1 1.71.29l4.05 4.06a2.33 2.33 0 0 1 0 3.3l-4.05 4.06a1 1 0 0 1-1.42 0z"
                      fill="currentColor"
                      fillRule="evenodd"
                    ></path>
                  </svg>
                </a>
              </div>
              <div className="sc-jGKxIK bnCkIr">
                <div className="sc-jMakVo gehSvw">
                  <div
                    className="LazyLoad is-visible"
                    style={{ height: "24px", width: "24px" }}
                  >
                    <img
                      src={img}
                      crossOrigin="anonymous"
                      alt={img}
                      className="sc-gEvEer fFqlxh"
                    />
                  </div>
                  <a
                    // Assuming 'tx' is meant to indicate a button style
                    style={{ display: "inline-block", height: "min-content" }} // Replacing 'sx' and '__css' with inline styles
                    className="sc-gEvEer KEfyk button"
                    href="/swap?base=.&amp;quote=cc8d1b026353022abbfcc2e1e71159f9e308d9c6e905ac1db24c7fb6.50617269627573"
                  >
                    <div className="sc-gEvEer jVsTBr">PBX</div>
                  </a>
                  <div className="sc-gEvEer jVsTBr">0.002472</div>
                  <p className="sc-iMTnTL iNflre">↑0.68%</p>
                  <div
                    className="LazyLoad is-visible"
                    style={{ height: "24px", width: "24px" }}
                  >
                    <img
                      src="https://tokens.muesliswap.com/static/img/tokens/5d16cc1a177b5d9ba9cfa9793b07e60f1fb70fea1f8aef064415d114.494147_scaled_100.webp"
                      crossOrigin="anonymous"
                      alt="https://tokens.muesliswap.com/static/img/tokens/5d16cc1a177b5d9ba9cfa9793b07e60f1fb70fea1f8aef064415d114.494147_scaled_100.webp"
                      className="sc-gEvEer fFqlxh"
                    />
                  </div>
                  <a
                    style={{ display: "inline-block", height: "min-content" }} // Replacing 'tx', 'variant', 'sx', and '__css' with inline styles
                    className="sc-gEvEer KEfyk button"
                    href="/swap?base=.&amp;quote=5d16cc1a177b5d9ba9cfa9793b07e60f1fb70fea1f8aef064415d114.494147"
                  >
                    <div className="sc-gEvEer jVsTBr">IAG</div>
                  </a>
                  <div className="sc-gEvEer jVsTBr">0.334745</div>
                  <p className="sc-iMTnTL iNflre">0.00%</p>
                  <div
                    className="LazyLoad is-visible"
                    style={{ height: "24px", width: "24px" }}
                  >
                    <img
                      src="https://tokens.muesliswap.com/static/img/tokens/1d7f33bd23d85e1a25d87d86fac4f199c3197a2f7afeb662a0f34e1e.776f726c646d6f62696c65746f6b656e_scaled_100.webp"
                      crossOrigin="anonymous"
                      alt="https://tokens.muesliswap.com/static/img/tokens/1d7f33bd23d85e1a25d87d86fac4f199c3197a2f7afeb662a0f34e1e.776f726c646d6f62696c65746f6b656e_scaled_100.webp"
                      className="sc-gEvEer fFqlxh"
                    />
                  </div>
                  <a
                    style={{ display: "inline-block", height: "min-content" }} // Replacing 'tx', 'variant', 'sx', and '__css' with inline styles
                    className="sc-gEvEer KEfyk"
                    href="/swap?base=.&amp;quote=1d7f33bd23d85e1a25d87d86fac4f199c3197a2f7afeb662a0f34e1e.776f726c646d6f62696c65746f6b656e"
                  >
                    <div className="sc-gEvEer jVsTBr">WMT</div>
                  </a>
                  <div className="sc-gEvEer jVsTBr">0.576245</div>
                  <p className="sc-iMTnTL iNflre">0.00%</p>
                </div>
              </div>
            </div>
            <div className="sc-krNlru iDYjIk">
              <div className="sc-jaXxmE eizVwN">
                <h2 className="sc-gEvEer fUmKfJ">📈 Top Gainers</h2>
                <a
                  aria-label="Show 📈 Top Gainers"
                  className="sc-hwdzOV jzRhqN"
                  href="/markets?sortBy=priceChange1d&amp;sortDir=true"
                >
                  <svg
                    height="12"
                    viewBox="0 0 7 12"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M.3 11.7a1 1 0 0 1 0-1.4l4.05-4.06a.33.33 0 0 0 0-.48L.29 1.71A1 1 0 1 1 1.71.29l4.05 4.06a2.33 2.33 0 0 1 0 3.3l-4.05 4.06a1 1 0 0 1-1.42 0z"
                      fill="currentColor"
                      fillRule="evenodd"
                    ></path>
                  </svg>
                </a>
              </div>
              <div className="sc-jGKxIK bnCkIr">
                <div className="sc-jMakVo gehSvw">
                  <div
                    className="LazyLoad is-visible"
                    style={{ height: "24px", width: "24px" }}
                  >
                    <img
                      src="https://tokens.muesliswap.com/static/img/tokens/5dac8536653edc12f6f5e1045d8164b9f59998d3bdc300fc92843489.4e4d4b52_scaled_100.webp"
                      crossOrigin="anonymous"
                      alt="https://tokens.muesliswap.com/static/img/tokens/5dac8536653edc12f6f5e1045d8164b9f59998d3bdc300fc92843489.4e4d4b52_scaled_100.webp"
                      className="sc-gEvEer fFqlxh"
                    />
                  </div>
                  <a
                    style={{ display: "inline-block", height: "min-content" }} // Replacing 'tx', 'variant', 'sx', and '__css' with inline styles
                    className="sc-gEvEer KEfyk"
                    href="/swap?base=.&amp;quote=5dac8536653edc12f6f5e1045d8164b9f59998d3bdc300fc92843489.4e4d4b52"
                  >
                    <div className="sc-gEvEer jVsTBr">NMKR</div>
                  </a>
                  <div className="sc-gEvEer jVsTBr">0.003189</div>
                  <p className="sc-iMTnTL iNflre">↑1.85%</p>
                  <div
                    className="LazyLoad is-visible"
                    style={{ height: "24px", width: "24px" }}
                  >
                    <img
                      src="https://tokens.muesliswap.com/static/img/tokens/edfd7a1d77bcb8b884c474bdc92a16002d1fb720e454fa6e99344479.4e5458_scaled_100.webp"
                      crossOrigin="anonymous"
                      alt="https://tokens.muesliswap.com/static/img/tokens/edfd7a1d77bcb8b884c474bdc92a16002d1fb720e454fa6e99344479.4e5458_scaled_100.webp"
                      className="sc-gEvEer fFqlxh"
                    />
                  </div>
                  <a
                    style={{ display: "inline-block", height: "min-content" }} // Replacing 'tx', 'variant', 'sx', and '__css' with inline styles
                    className="sc-gEvEer KEfyk"
                    href="/swap?base=.&amp;quote=edfd7a1d77bcb8b884c474bdc92a16002d1fb720e454fa6e99344479.4e5458"
                  >
                    <div className="sc-gEvEer jVsTBr">NTX</div>
                  </a>
                  <div className="sc-gEvEer jVsTBr">0.09295</div>
                  <p className="sc-iMTnTL iNflre">↑1.21%</p>
                  <div
                    className="LazyLoad is-visible"
                    style={{ height: "24px", width: "24px" }}
                  >
                    <img
                      src={img}
                      crossOrigin="anonymous"
                      alt={img}
                      className="sc-gEvEer fFqlxh"
                    />
                  </div>
                  <a
                    className="sc-gEvEer KEfyk"
                    href="/swap?base=.&amp;quote=cc8d1b026353022abbfcc2e1e71159f9e308d9c6e905ac1db24c7fb6.50617269627573"
                  >
                    <div className="sc-gEvEer jVsTBr">PBX</div>
                  </a>
                  <div className="sc-gEvEer jVsTBr">0.002472</div>
                  <p className="sc-iMTnTL iNflre">↑0.68%</p>
                </div>
              </div>
            </div>
            <div className="sc-krNlru iDYjIk">
              <div className="sc-jaXxmE eizVwN">
                <h2 className="sc-gEvEer fUmKfJ">📉 Top Losers</h2>
                <a
                  aria-label="Show 📉 Top Losers"
                  className="sc-hwdzOV jzRhqN"
                  href="/markets?sortBy=priceChange1d&amp;sortDir=false"
                >
                  <svg
                    height="12"
                    viewBox="0 0 7 12"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M.3 11.7a1 1 0 0 1 0-1.4l4.05-4.06a.33.33 0 0 0 0-.48L.29 1.71A1 1 0 1 1 1.71.29l4.05 4.06a2.33 2.33 0 0 1 0 3.3l-4.05 4.06a1 1 0 0 1-1.42 0z"
                      fill="currentColor"
                      fillRule="evenodd"
                    ></path>
                  </svg>
                </a>
              </div>
              <div className="sc-jGKxIK bnCkIr">
                <div className="sc-jMakVo gehSvw">
                  <div
                    className="LazyLoad is-visible"
                    style={{ height: "24px", width: "24px" }}
                  >
                    <img
                      src="https://tokens.muesliswap.com/static/img/tokens/43b07d4037f0d75ee10f9863097463fc02ff3c0b8b705ae61d9c75bf.4d796e746820546f6b656e_scaled_100.webp"
                      crossOrigin="anonymous"
                      alt="https://tokens.muesliswap.com/static/img/tokens/43b07d4037f0d75ee10f9863097463fc02ff3c0b8b705ae61d9c75bf.4d796e746820546f6b656e_scaled_100.webp"
                      className="sc-gEvEer fFqlxh"
                    />
                  </div>
                  <a
                    // Assuming 'tx' is for button styling
                    style={{ display: "inline-block", height: "min-content" }} // Replacing 'sx' and '__css' with inline styles
                    className="sc-gEvEer KEfyk button"
                    href="/swap?base=.&amp;quote=43b07d4037f0d75ee10f9863097463fc02ff3c0b8b705ae61d9c75bf.4d796e746820546f6b656e"
                  >
                    <div className="sc-gEvEer jVsTBr">MNT</div>
                  </a>
                  <div className="sc-gEvEer jVsTBr">0</div>
                  <p className="sc-iMTnTL dJNOmi">↓16.71%</p>
                  <div
                    className="LazyLoad is-visible"
                    style={{ height: "24px", width: "24px" }}
                  >
                    <img
                      src="https://tokens.muesliswap.com/static/img/tokens/8f9c32977d2bacb87836b64f7811e99734c6368373958da20172afba.4d5949454c44_scaled_100.webp"
                      crossOrigin="anonymous"
                      alt="https://tokens.muesliswap.com/static/img/tokens/8f9c32977d2bacb87836b64f7811e99734c6368373958da20172afba.4d5949454c44_scaled_100.webp"
                      className="sc-gEvEer fFqlxh"
                    />
                  </div>
                  <a
                    style={{ display: "inline-block", height: "min-content" }} // Replacing 'tx', 'variant', 'sx', and '__css' with inline styles
                    className="sc-gEvEer KEfyk"
                    href="/swap?base=.&amp;quote=8f9c32977d2bacb87836b64f7811e99734c6368373958da20172afba.4d5949454c44"
                  >
                    <div className="sc-gEvEer jVsTBr">MYIELD</div>
                  </a>
                  <div className="sc-gEvEer jVsTBr">0.00845</div>
                  <p className="sc-iMTnTL dJNOmi">↓1.69%</p>
                  <div
                    className="LazyLoad is-visible"
                    style={{ height: "24px", width: "24px" }}
                  >
                    <img
                      src="https://tokens.muesliswap.com/static/img/tokens/8f52f6a88acf6127bc4758a16b6047afc4da7887feae121ec217b75a.534e4f57_scaled_100.webp"
                      crossOrigin="anonymous"
                      alt="https://tokens.muesliswap.com/static/img/tokens/8f52f6a88acf6127bc4758a16b6047afc4da7887feae121ec217b75a.534e4f57_scaled_100.webp"
                      className="sc-gEvEer fFqlxh"
                    />
                  </div>
                  <a
                    style={{ display: "inline-block", height: "min-content" }} // Replacing 'tx', 'variant', 'sx', and '__css' with inline styles
                    className="sc-gEvEer KEfyk"
                    href="/swap?base=.&amp;quote=8f52f6a88acf6127bc4758a16b6047afc4da7887feae121ec217b75a.534e4f57"
                  >
                    <div className="sc-gEvEer jVsTBr">SNOW</div>
                  </a>
                  <div className="sc-gEvEer jVsTBr">0.415899</div>
                  <p className="sc-iMTnTL dJNOmi">↓1.38%</p>
                </div>
              </div>
            </div>
          </div>
          <div className="sc-ihgnxF gQhDKT">
            <div className="sc-gFVvzn iyWJYO">
              <nav className="sc-brPLxw fwqDkj" style={{ display: "grid" }}>
                <div className="sc-gEvEer iKsSNG">
                  <a href="/markets?cat=all" style={{ textDecoration: "none" }}>
                    <div className="sc-gEvEer buwCeh">All</div>
                  </a>
                </div>
                <span className="sc-gEvEer jPzTRY"></span>
                <div className="sc-gEvEer iYpAFJ">
                  <a
                    href="/markets?cat=defi"
                    style={{ textDecoration: "none" }}
                  >
                    <div className="sc-gEvEer dTIZMK">DeFi</div>
                  </a>
                </div>
                <span className="sc-gEvEer jphDfC"></span>
                <div className="sc-gEvEer edKMow">
                  <a
                    href="/markets?cat=meme"
                    style={{ textDecoration: "none" }}
                  >
                    <div className="sc-gEvEer dTIZMK">Meme</div>
                  </a>
                </div>
                <span className="sc-gEvEer behshM"></span>
                <div className="sc-gEvEer eXjzAX">
                  <a
                    href="/markets?cat=bridge"
                    style={{ textDecoration: "none" }}
                  >
                    <div className="sc-gEvEer dTIZMK">Bridge</div>
                  </a>
                </div>
                <span className="sc-gEvEer eVxHgS"></span>
                <div className="sc-gEvEer cELjK">
                  <a
                    href="/markets?cat=other"
                    style={{ textDecoration: "none" }}
                  >
                    <div className="sc-gEvEer dTIZMK">Other</div>
                  </a>
                </div>
                <span className="sc-gEvEer cmUieI"></span>
              </nav>
              <div className="sc-gEvEer sc-eqUAAy jeSAXr fgprtA">
                <input
                  placeholder="Search token by name"
                  className="sc-gEvEer jdxaSg"
                  value=""
                />
                <svg
                  width="24px"
                  height="24px"
                  viewBox="0 0 24 24"
                  xmlns="http://www.w3.org/2000/svg"
                  color="text"
                  className="sc-gEvEer eILgRW"
                >
                  <g
                    id="Symbols"
                    stroke="none"
                    strokeWidth="1"
                    fill="none"
                    fillRule="evenodd"
                  >
                    <g id="Data-Display/Icons/cancel" fill="currentColor">
                      <path
                        d="M19.0742438,4.92574392 C15.1724148,1.02475203 8.82694036,1.02475203 4.92511138,4.92574392 C1.02496287,8.82673582 1.02496287,15.173825 4.92511138,19.0748169 C6.87602587,21.0249025 9.43804713,21.9994959 12.0001075,21.9994959 C14.5621678,21.9994959 17.1233293,21.0248634 19.0742438,19.0748169 C22.9752521,15.1738641 22.9752521,8.82673582 19.0742438,4.92574392 Z M16.9514186,15.5375449 C17.3425864,15.9287127 17.3425864,16.5610247 16.9514186,16.9521924 C16.7563272,17.1472839 16.5002228,17.2453455 16.2440714,17.2453455 C15.987967,17.2453455 15.7318157,17.1472839 15.5367242,16.9521924 L12.0000821,13.4145185 L8.46442473,16.9511607 C8.26834844,17.1462521 8.01219712,17.2443137 7.75707754,17.2443137 C7.50097311,17.2443137 7.24482179,17.1462521 7.04973034,16.9511607 C6.65856261,16.559993 6.65856261,15.9266961 7.04973034,15.5365132 L10.5853877,11.999871 L7.04874551,8.46322886 C6.65757777,8.07206112 6.65757777,7.43876426 7.04874551,7.04858137 C7.4389284,6.65741363 8.07222526,6.65741363 8.463393,7.04858137 L12.0000352,10.5852235 L15.5366773,7.04858137 C15.9278451,6.65741363 16.5601571,6.65741363 16.9513248,7.04858137 C17.3424926,7.43876426 17.3424926,8.07206112 16.9513248,8.46322886 L13.4146827,11.999871 L16.9514186,15.5375449 Z"
                        id="Shape"
                      ></path>
                    </g>
                  </g>
                </svg>
                <svg
                  width="24"
                  height="24"
                  xmlns="http://www.w3.org/2000/svg"
                  color="text"
                  // Removed invalid 'sx' property
                  className="sc-gEvEer ivHxAC"
                >
                  <path
                    d="m19.7 18.3-3.09-3.1a7 7 0 1 0-1.41 1.41l3.1 3.1a1 1 0 1 0 1.4-1.41ZM11.04 16a4.98 4.98 0 1 1 0-9.96 4.98 4.98 0 0 1 0 9.96Z"
                    fill="currentColor"
                    fillRule="evenodd"
                  ></path>
                </svg>
              </div>
              <div className="sc-gEvEer sc-eqUAAy iLeEgO fgprtA">
                <div className="sc-gEvEer sc-eqUAAy jQCsrt fgprtA">
                  <p className="sc-gEvEer iZRsmm">Only&nbsp;verified</p>
                  <label className="sc-bmzYkS cgGqnT">
                    <input
                      type="checkbox"
                      className="sc-eeDRCY fotkZP"
                      checked=""
                    />
                    <span className="sc-koXPp kAqvkZ"></span>
                  </label>
                </div>
                <div className="sc-gEvEer sc-eqUAAy jQCsrt fgprtA">
                  <div className="sc-gEvEer kpVmdW">Bases:</div>
                  <div className="sc-gEvEer sc-eqUAAy bUvvFL fgprtA">
                    <button
                      disabled=""
                      className="sc-gEvEer hrLZDX sc-gEvEer sc-eqUAAy jPcUWI fgprtA"
                    >
                      <div
                        className="LazyLoad is-visible"
                        style={{ height: "20px", width: "20px" }}
                      >
                        <img
                          src="/static/media/ada.ae3e320f25e324286ae2.webp"
                          crossOrigin="anonymous"
                          alt="/static/media/ada.ae3e320f25e324286ae2.webp"
                          className="sc-gEvEer iYGsOJ"
                        />
                      </div>
                      <div font-weight="semiBold" className="sc-gEvEer fffdtI">
                        ADA
                      </div>
                    </button>
                    <button className="sc-gEvEer hrLZDX sc-gEvEer sc-eqUAAy jPcUWI fgprtA">
                      <div
                        className="LazyLoad is-visible"
                        style={{ height: "20px", width: "20px" }}
                      >
                        <img
                          src="https://tokens.muesliswap.com/static/img/tokens/8db269c3ec630e06ae29f74bc39edd1f87c819f1056206e879a1cd61.446a65644d6963726f555344.png"
                          crossOrigin="anonymous"
                          alt="https://tokens.muesliswap.com/static/img/tokens/8db269c3ec630e06ae29f74bc39edd1f87c819f1056206e879a1cd61.446a65644d6963726f555344.png"
                          className="sc-gEvEer eBzULB"
                        />
                      </div>
                      <div opacity="0.8" className="sc-gEvEer dVZlJT">
                        DJED
                      </div>
                    </button>
                    <button className="sc-gEvEer hrLZDX sc-gEvEer sc-eqUAAy jPcUWI fgprtA">
                      <div
                        className="LazyLoad is-visible"
                        style={{ height: "20px", width: "20px" }}
                      >
                        <img
                          src="https://tokens.muesliswap.com/static/img/tokens/f66d78b4a3cb3d37afa0ec36461e51ecbde00f26c8f0a68f94b69880.69555344.png"
                          crossOrigin="anonymous"
                          alt="https://tokens.muesliswap.com/static/img/tokens/f66d78b4a3cb3d37afa0ec36461e51ecbde00f26c8f0a68f94b69880.69555344.png"
                          className="sc-gEvEer eBzULB"
                        />
                      </div>
                      <div opacity="0.8" className="sc-gEvEer dVZlJT">
                        iUSD
                      </div>
                    </button>
                  </div>
                </div>
              </div>
            </div>
            <table role="table" className="sc-hCPjZK dosTKk">
              <thead className="sc-cfxfcM ihWKps">
                <tr role="row">
                  <th
                    colSpan="1"
                    role="columnheader"
                    className="sc-gFAWRd itlcvQ"
                  >
                    <span hidden="" className="sc-ddjGPC fRRtfN">
                      <div className="sc-aXZVg jrzA-dM">
                        <svg
                          width="10"
                          height="6"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path
                            d="M.76 6h8.48a.76.76 0 0 0 .7-.5.81.81 0 0 0-.17-.85L5.54.23A.74.74 0 0 0 5 0a.74.74 0 0 0-.54.23L.22 4.65a.81.81 0 0 0-.16.86.76.76 0 0 0 .7.49Z"
                            fill="currentColor"
                            fillRule="evenodd"
                          ></path>
                        </svg>
                      </div>
                    </span>
                  </th>
                  <th
                    colSpan="1"
                    role="columnheader"
                    className="sc-gFAWRd itlcvQ"
                  >
                    Name
                    <span hidden="" className="sc-ddjGPC fRRtfN">
                      <div className="sc-aXZVg jrzA-dM">
                        <svg
                          width="10"
                          height="6"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path
                            d="M.76 6h8.48a.76.76 0 0 0 .7-.5.81.81 0 0 0-.17-.85L5.54.23A.74.74 0 0 0 5 0a.74.74 0 0 0-.54.23L.22 4.65a.81.81 0 0 0-.16.86.76.76 0 0 0 .7.49Z"
                            fill="currentColor"
                            fillRule="evenodd"
                          ></path>
                        </svg>
                      </div>
                    </span>
                  </th>
                  <th
                    colSpan="1"
                    role="columnheader"
                    className="sc-gFAWRd itlcvQ"
                  >
                    Price (ADA)
                    <span hidden="" className="sc-ddjGPC fRRtfN">
                      <div className="sc-aXZVg jrzA-dM">
                        <svg
                          width="10"
                          height="6"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path
                            d="M.76 6h8.48a.76.76 0 0 0 .7-.5.81.81 0 0 0-.17-.85L5.54.23A.74.74 0 0 0 5 0a.74.74 0 0 0-.54.23L.22 4.65a.81.81 0 0 0-.16.86.76.76 0 0 0 .7.49Z"
                            fill="currentColor"
                            fillRule="evenodd"
                          ></path>
                        </svg>
                      </div>
                    </span>
                  </th>
                  <th
                    colSpan="1"
                    role="columnheader"
                    className="sc-gFAWRd itlcvQ"
                  >
                    24h %
                    <span hidden="" className="sc-ddjGPC fRRtfN">
                      <div className="sc-aXZVg jrzA-dM">
                        <svg
                          width="10"
                          height="6"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path
                            d="M.76 6h8.48a.76.76 0 0 0 .7-.5.81.81 0 0 0-.17-.85L5.54.23A.74.74 0 0 0 5 0a.74.74 0 0 0-.54.23L.22 4.65a.81.81 0 0 0-.16.86.76.76 0 0 0 .7.49Z"
                            fill="currentColor"
                            fillRule="evenodd"
                          ></path>
                        </svg>
                      </div>
                    </span>
                  </th>
                  <th
                    colSpan="1"
                    role="columnheader"
                    className="sc-gFAWRd itlcvQ"
                  >
                    7d %
                    <span hidden="" className="sc-ddjGPC fRRtfN">
                      <div className="sc-aXZVg jrzA-dM">
                        <svg
                          width="10"
                          height="6"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path
                            d="M.76 6h8.48a.76.76 0 0 0 .7-.5.81.81 0 0 0-.17-.85L5.54.23A.74.74 0 0 0 5 0a.74.74 0 0 0-.54.23L.22 4.65a.81.81 0 0 0-.16.86.76.76 0 0 0 .7.49Z"
                            fill="currentColor"
                            fillRule="evenodd"
                          ></path>
                        </svg>
                      </div>
                    </span>
                  </th>
                  <th
                    colSpan="1"
                    role="columnheader"
                    className="sc-gFAWRd itlcvQ"
                  >
                    Volume 24h (ADA)
                    <span className="sc-ddjGPC dtONNa">
                      <div className="sc-aXZVg jrzA-dM">
                        <svg
                          width="10"
                          height="6"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path
                            d="M.76 6h8.48a.76.76 0 0 0 .7-.5.81.81 0 0 0-.17-.85L5.54.23A.74.74 0 0 0 5 0a.74.74 0 0 0-.54.23L.22 4.65a.81.81 0 0 0-.16.86.76.76 0 0 0 .7.49Z"
                            fill="currentColor"
                            fillRule="evenodd"
                          ></path>
                        </svg>
                      </div>
                    </span>
                  </th>
                  <th
                    colSpan="1"
                    role="columnheader"
                    className="sc-gFAWRd itlcvQ"
                  >
                    Market Cap (ADA)
                    <span hidden="" className="sc-ddjGPC fRRtfN">
                      <div className="sc-aXZVg jrzA-dM">
                        <svg
                          width="10"
                          height="6"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path
                            d="M.76 6h8.48a.76.76 0 0 0 .7-.5.81.81 0 0 0-.17-.85L5.54.23A.74.74 0 0 0 5 0a.74.74 0 0 0-.54.23L.22 4.65a.81.81 0 0 0-.16.86.76.76 0 0 0 .7.49Z"
                            fill="currentColor"
                            fillRule="evenodd"
                          ></path>
                        </svg>
                      </div>
                    </span>
                  </th>
                  <th
                    colSpan="1"
                    role="columnheader"
                    className="sc-gFAWRd itlcvQ"
                  >
                    Last 10 Days
                    <span hidden="" className="sc-ddjGPC fRRtfN">
                      <div className="sc-aXZVg jrzA-dM">
                        <svg
                          width="10"
                          height="6"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path
                            d="M.76 6h8.48a.76.76 0 0 0 .7-.5.81.81 0 0 0-.17-.85L5.54.23A.74.74 0 0 0 5 0a.74.74 0 0 0-.54.23L.22 4.65a.81.81 0 0 0-.16.86.76.76 0 0 0 .7.49Z"
                            fill="currentColor"
                            fillRule="evenodd"
                          ></path>
                        </svg>
                      </div>
                    </span>
                  </th>
                </tr>
              </thead>
              <tbody role="rowgroup">
                <tr role="row" className="sc-gmPhUn FCgni">
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <label className="sc-kAkpmW cRPsKG">
                      <input type="checkbox" className="sc-cmaqmh iwhYbG" />
                      <svg
                        width="20"
                        height="20"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M5.91 18.32a1.68 1.68 0 0 1-1.65-1.95l.65-3.97-2.77-2.86a1.68 1.68 0 0 1 .95-2.82l3.74-.58 1.65-3.53a1.69 1.69 0 0 1 3.03 0l1.65 3.53 3.75.58a1.68 1.68 0 0 1 .95 2.82L15.1 12.4l.64 3.98a1.67 1.67 0 0 1-2.47 1.74L10 16.29l-3.27 1.82a1.67 1.67 0 0 1-.82.21Zm4.09-3.7c.28 0 .56.08.81.22l3.27 1.81-.63-3.98a1.69 1.69 0 0 1 .45-1.44l2.76-2.85-3.75-.6a1.67 1.67 0 0 1-1.26-.94l-1.64-3.52-1.66 3.52a1.67 1.67 0 0 1-1.26.95l-3.75.57 2.76 2.87c.37.38.54.91.45 1.44l-.64 3.97 3.28-1.8c.25-.14.53-.21.81-.22Z"
                          fill="currentColor"
                          fillRule="evenodd"
                        ></path>
                      </svg>
                    </label>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <a
                      className="sc-iHbSHJ bwYSnM"
                      href="/swap?base=.&amp;quote=cc8d1b026353022abbfcc2e1e71159f9e308d9c6e905ac1db24c7fb6.50617269627573"
                    >
                      <div
                        className="LazyLoad is-visible"
                        style={{ height: "32px", width: "59.2px" }}
                      >
                        <div
                          width="59.2px"
                          height="32px"
                          className="sc-cWSHoV dazaWi"
                        >
                          <img
                            src={img}
                            crossOrigin="anonymous"
                            alt={img}
                            className="sc-gEvEer ceiwSH"
                          />
                          <img
                            src="/static/media/ada.ae3e320f25e324286ae2.webp"
                            crossOrigin="anonymous"
                            alt=""
                            className="sc-gEvEer gUHNA-d"
                          />
                        </div>
                      </div>
                      <span className="sc-Nxspf ebZgeq">PBX</span>
                      <span className="sc-klVQfs cErbak">Verified</span>
                      <span className="sc-klVQfs bRcUix"> DeFi </span>
                    </a>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">0.002472</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      8.739E-4$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div value="0.6848155848154569" className="sc-uVWWZ lcUsoT">
                      ↑&nbsp;0.68%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div value="14.309946917302607" className="sc-uVWWZ lcUsoT">
                      ↑&nbsp;14.31%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">12,377</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      4,375$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">24,723,836</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      8,739,011$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div className="sc-gEvEer fiARsy">
                      <canvas
                        height="48"
                        width="112"
                        id="markets-graph-cell-cc8d1b026353022abbfcc2e1e71159f9e308d9c6e905ac1db24c7fb6.50617269627573"
                        style={{ width: "112px", height: "48px" }}
                      ></canvas>
                    </div>
                  </td>
                </tr>
                <tr role="row" className="sc-gmPhUn FCgni">
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <label className="sc-kAkpmW cRPsKG">
                      <input type="checkbox" className="sc-cmaqmh iwhYbG" />
                      <svg
                        width="20"
                        height="20"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M5.91 18.32a1.68 1.68 0 0 1-1.65-1.95l.65-3.97-2.77-2.86a1.68 1.68 0 0 1 .95-2.82l3.74-.58 1.65-3.53a1.69 1.69 0 0 1 3.03 0l1.65 3.53 3.75.58a1.68 1.68 0 0 1 .95 2.82L15.1 12.4l.64 3.98a1.67 1.67 0 0 1-2.47 1.74L10 16.29l-3.27 1.82a1.67 1.67 0 0 1-.82.21Zm4.09-3.7c.28 0 .56.08.81.22l3.27 1.81-.63-3.98a1.69 1.69 0 0 1 .45-1.44l2.76-2.85-3.75-.6a1.67 1.67 0 0 1-1.26-.94l-1.64-3.52-1.66 3.52a1.67 1.67 0 0 1-1.26.95l-3.75.57 2.76 2.87c.37.38.54.91.45 1.44l-.64 3.97 3.28-1.8c.25-.14.53-.21.81-.22Z"
                          fill="currentColor"
                          fillRule="evenodd"
                        ></path>
                      </svg>
                    </label>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <a
                      className="sc-iHbSHJ bwYSnM"
                      href="/swap?base=.&amp;quote=5d16cc1a177b5d9ba9cfa9793b07e60f1fb70fea1f8aef064415d114.494147"
                    >
                      <div
                        className="LazyLoad is-visible"
                        style={{ height: "32px", width: "59.2px" }}
                      >
                        <div
                          width="59.2px"
                          height="32px"
                          className="sc-cWSHoV dazaWi"
                        >
                          <img
                            src="https://tokens.muesliswap.com/static/img/tokens/5d16cc1a177b5d9ba9cfa9793b07e60f1fb70fea1f8aef064415d114.494147_scaled_100.webp"
                            crossOrigin="anonymous"
                            alt="https://tokens.muesliswap.com/static/img/tokens/5d16cc1a177b5d9ba9cfa9793b07e60f1fb70fea1f8aef064415d114.494147_scaled_100.webp"
                            className="sc-gEvEer ceiwSH"
                          />
                          <img
                            src="/static/media/ada.ae3e320f25e324286ae2.webp"
                            crossOrigin="anonymous"
                            alt=""
                            className="sc-gEvEer gUHNA-d"
                          />
                        </div>
                      </div>
                      <span className="sc-Nxspf ebZgeq">IAG</span>
                      <span className="sc-klVQfs cErbak">Verified</span>
                    </a>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">0.334745</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      0.118$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div value="0" className="sc-uVWWZ eJsMUQ">
                      &nbsp;0%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div value="34.124035727544786" className="sc-uVWWZ lcUsoT">
                      ↑&nbsp;34.12%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">11,991</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      4,238$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">130,572,696</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      46,152,878$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div className="sc-gEvEer fiARsy">
                      <canvas
                        height="48"
                        width="112"
                        id="markets-graph-cell-5d16cc1a177b5d9ba9cfa9793b07e60f1fb70fea1f8aef064415d114.494147"
                        style={{ width: "112px", height: "48px" }}
                      ></canvas>
                    </div>
                  </td>
                </tr>
                <tr role="row" className="sc-gmPhUn FCgni">
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <label className="sc-kAkpmW cRPsKG">
                      <input type="checkbox" className="sc-cmaqmh iwhYbG" />
                      <svg
                        width="20"
                        height="20"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M5.91 18.32a1.68 1.68 0 0 1-1.65-1.95l.65-3.97-2.77-2.86a1.68 1.68 0 0 1 .95-2.82l3.74-.58 1.65-3.53a1.69 1.69 0 0 1 3.03 0l1.65 3.53 3.75.58a1.68 1.68 0 0 1 .95 2.82L15.1 12.4l.64 3.98a1.67 1.67 0 0 1-2.47 1.74L10 16.29l-3.27 1.82a1.67 1.67 0 0 1-.82.21Zm4.09-3.7c.28 0 .56.08.81.22l3.27 1.81-.63-3.98a1.69 1.69 0 0 1 .45-1.44l2.76-2.85-3.75-.6a1.67 1.67 0 0 1-1.26-.94l-1.64-3.52-1.66 3.52a1.67 1.67 0 0 1-1.26.95l-3.75.57 2.76 2.87c.37.38.54.91.45 1.44l-.64 3.97 3.28-1.8c.25-.14.53-.21.81-.22Z"
                          fill="currentColor"
                          fillRule="evenodd"
                        ></path>
                      </svg>
                    </label>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <a
                      className="sc-iHbSHJ bwYSnM"
                      href="/swap?base=.&amp;quote=1d7f33bd23d85e1a25d87d86fac4f199c3197a2f7afeb662a0f34e1e.776f726c646d6f62696c65746f6b656e"
                    >
                      <div
                        className="LazyLoad is-visible"
                        style={{ height: "32px", width: "59.2px" }}
                      >
                        <div
                          width="59.2px"
                          height="32px"
                          className="sc-cWSHoV dazaWi"
                        >
                          <img
                            src="https://tokens.muesliswap.com/static/img/tokens/1d7f33bd23d85e1a25d87d86fac4f199c3197a2f7afeb662a0f34e1e.776f726c646d6f62696c65746f6b656e_scaled_100.webp"
                            crossOrigin="anonymous"
                            alt="https://tokens.muesliswap.com/static/img/tokens/1d7f33bd23d85e1a25d87d86fac4f199c3197a2f7afeb662a0f34e1e.776f726c646d6f62696c65746f6b656e_scaled_100.webp"
                            className="sc-gEvEer ceiwSH"
                          />
                          <img
                            src="/static/media/ada.ae3e320f25e324286ae2.webp"
                            crossOrigin="anonymous"
                            alt=""
                            className="sc-gEvEer gUHNA-d"
                          />
                        </div>
                      </div>
                      <span className="sc-Nxspf ebZgeq">WMT</span>
                      <span className="sc-klVQfs cErbak">Verified</span>
                      <span className="sc-klVQfs bRcUix"> Popular </span>
                      <span className="sc-klVQfs bRcUix"> Other </span>
                    </a>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">0.576245</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      0.204$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div value="0" className="sc-uVWWZ eJsMUQ">
                      &nbsp;0%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div
                      value="-1.6559370498554715"
                      className="sc-uVWWZ ejrJPX"
                    >
                      ↓&nbsp;1.66%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">7,863</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      2,779$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">131,303,073</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      46,411,041$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div className="sc-gEvEer fiARsy">
                      <canvas
                        height="48"
                        width="112"
                        id="markets-graph-cell-1d7f33bd23d85e1a25d87d86fac4f199c3197a2f7afeb662a0f34e1e.776f726c646d6f62696c65746f6b656e"
                        style={{ width: "112px", height: "48px" }}
                      ></canvas>
                    </div>
                  </td>
                </tr>
                <tr role="row" className="sc-gmPhUn FCgni">
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <label className="sc-kAkpmW cRPsKG">
                      <input type="checkbox" className="sc-cmaqmh iwhYbG" />
                      <svg
                        width="20"
                        height="20"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M5.91 18.32a1.68 1.68 0 0 1-1.65-1.95l.65-3.97-2.77-2.86a1.68 1.68 0 0 1 .95-2.82l3.74-.58 1.65-3.53a1.69 1.69 0 0 1 3.03 0l1.65 3.53 3.75.58a1.68 1.68 0 0 1 .95 2.82L15.1 12.4l.64 3.98a1.67 1.67 0 0 1-2.47 1.74L10 16.29l-3.27 1.82a1.67 1.67 0 0 1-.82.21Zm4.09-3.7c.28 0 .56.08.81.22l3.27 1.81-.63-3.98a1.69 1.69 0 0 1 .45-1.44l2.76-2.85-3.75-.6a1.67 1.67 0 0 1-1.26-.94l-1.64-3.52-1.66 3.52a1.67 1.67 0 0 1-1.26.95l-3.75.57 2.76 2.87c.37.38.54.91.45 1.44l-.64 3.97 3.28-1.8c.25-.14.53-.21.81-.22Z"
                          fill="currentColor"
                          fillRule="evenodd"
                        ></path>
                      </svg>
                    </label>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <a
                      className="sc-iHbSHJ bwYSnM"
                      href="/swap?base=.&amp;quote=279c909f348e533da5808898f87f9a14bb2c3dfbbacccd631d927a3f.534e454b"
                    >
                      <div
                        className="LazyLoad is-visible"
                        style={{ height: "32px", width: "59.2px" }}
                      >
                        <div
                          width="59.2px"
                          height="32px"
                          className="sc-cWSHoV dazaWi"
                        >
                          <img
                            src="https://tokens.muesliswap.com/static/img/tokens/279c909f348e533da5808898f87f9a14bb2c3dfbbacccd631d927a3f.534e454b_scaled_100.webp"
                            crossOrigin="anonymous"
                            alt="https://tokens.muesliswap.com/static/img/tokens/279c909f348e533da5808898f87f9a14bb2c3dfbbacccd631d927a3f.534e454b_scaled_100.webp"
                            className="sc-gEvEer ceiwSH"
                          />
                          <img
                            src="/static/media/ada.ae3e320f25e324286ae2.webp"
                            crossOrigin="anonymous"
                            alt=""
                            className="sc-gEvEer gUHNA-d"
                          />
                        </div>
                      </div>
                      <span className="sc-Nxspf ebZgeq">SNEK</span>
                      <span className="sc-klVQfs cErbak">Verified</span>
                      <span className="sc-klVQfs bRcUix"> Popular </span>
                      <span className="sc-klVQfs bRcUix"> Meme </span>
                    </a>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">0.002091</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      7.391E-4$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div value="0" className="sc-uVWWZ eJsMUQ">
                      &nbsp;0%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div value="2.2880112709918965" className="sc-uVWWZ lcUsoT">
                      ↑&nbsp;2.29%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">6,832</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      2,415$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">155,000,882</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      54,787,387$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div className="sc-gEvEer fiARsy">
                      <canvas
                        height="48"
                        width="112"
                        id="markets-graph-cell-279c909f348e533da5808898f87f9a14bb2c3dfbbacccd631d927a3f.534e454b"
                        style={{ width: "112px", height: "48px" }}
                      ></canvas>
                    </div>
                  </td>
                </tr>
                <tr role="row" className="sc-gmPhUn FCgni">
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <label className="sc-kAkpmW cRPsKG">
                      <input type="checkbox" className="sc-cmaqmh iwhYbG" />
                      <svg
                        width="20"
                        height="20"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M5.91 18.32a1.68 1.68 0 0 1-1.65-1.95l.65-3.97-2.77-2.86a1.68 1.68 0 0 1 .95-2.82l3.74-.58 1.65-3.53a1.69 1.69 0 0 1 3.03 0l1.65 3.53 3.75.58a1.68 1.68 0 0 1 .95 2.82L15.1 12.4l.64 3.98a1.67 1.67 0 0 1-2.47 1.74L10 16.29l-3.27 1.82a1.67 1.67 0 0 1-.82.21Zm4.09-3.7c.28 0 .56.08.81.22l3.27 1.81-.63-3.98a1.69 1.69 0 0 1 .45-1.44l2.76-2.85-3.75-.6a1.67 1.67 0 0 1-1.26-.94l-1.64-3.52-1.66 3.52a1.67 1.67 0 0 1-1.26.95l-3.75.57 2.76 2.87c.37.38.54.91.45 1.44l-.64 3.97 3.28-1.8c.25-.14.53-.21.81-.22Z"
                          fill="currentColor"
                          fillRule="evenodd"
                        ></path>
                      </svg>
                    </label>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <a
                      className="sc-iHbSHJ bwYSnM"
                      href="/swap?base=.&amp;quote=f43a62fdc3965df486de8a0d32fe800963589c41b38946602a0dc535.41474958"
                    >
                      <div
                        className="LazyLoad is-visible"
                        style={{ height: "32px", width: "59.2px" }}
                      >
                        <div
                          width="59.2px"
                          height="32px"
                          className="sc-cWSHoV dazaWi"
                        >
                          <img
                            src="https://tokens.muesliswap.com/static/img/tokens/f43a62fdc3965df486de8a0d32fe800963589c41b38946602a0dc535.41474958_scaled_100.webp"
                            crossOrigin="anonymous"
                            alt="https://tokens.muesliswap.com/static/img/tokens/f43a62fdc3965df486de8a0d32fe800963589c41b38946602a0dc535.41474958_scaled_100.webp"
                            className="sc-gEvEer ceiwSH"
                          />
                          <img
                            src="/static/media/ada.ae3e320f25e324286ae2.webp"
                            crossOrigin="anonymous"
                            alt=""
                            className="sc-gEvEer gUHNA-d"
                          />
                        </div>
                      </div>
                      <span className="sc-Nxspf ebZgeq">AGIX</span>
                      <span className="sc-klVQfs cErbak">Verified</span>
                      <span className="sc-klVQfs bRcUix"> Other </span>
                    </a>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">1.943124</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      0.687$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div
                      value="-0.40867356055985793"
                      className="sc-uVWWZ ejrJPX"
                    >
                      ↓&nbsp;0.41%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div value="13.56304569713469" className="sc-uVWWZ lcUsoT">
                      ↑&nbsp;13.56%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">6,244</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      2,207$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">1,006,177,474</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      355,648,521$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div className="sc-gEvEer fiARsy">
                      <canvas
                        height="48"
                        width="112"
                        id="markets-graph-cell-f43a62fdc3965df486de8a0d32fe800963589c41b38946602a0dc535.41474958"
                        style={{ width: "112px", height: "48px" }}
                      ></canvas>
                    </div>
                  </td>
                </tr>
                <tr role="row" className="sc-gmPhUn FCgni">
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <label className="sc-kAkpmW cRPsKG">
                      <input type="checkbox" className="sc-cmaqmh iwhYbG" />
                      <svg
                        width="20"
                        height="20"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M5.91 18.32a1.68 1.68 0 0 1-1.65-1.95l.65-3.97-2.77-2.86a1.68 1.68 0 0 1 .95-2.82l3.74-.58 1.65-3.53a1.69 1.69 0 0 1 3.03 0l1.65 3.53 3.75.58a1.68 1.68 0 0 1 .95 2.82L15.1 12.4l.64 3.98a1.67 1.67 0 0 1-2.47 1.74L10 16.29l-3.27 1.82a1.67 1.67 0 0 1-.82.21Zm4.09-3.7c.28 0 .56.08.81.22l3.27 1.81-.63-3.98a1.69 1.69 0 0 1 .45-1.44l2.76-2.85-3.75-.6a1.67 1.67 0 0 1-1.26-.94l-1.64-3.52-1.66 3.52a1.67 1.67 0 0 1-1.26.95l-3.75.57 2.76 2.87c.37.38.54.91.45 1.44l-.64 3.97 3.28-1.8c.25-.14.53-.21.81-.22Z"
                          fill="currentColor"
                          fillRule="evenodd"
                        ></path>
                      </svg>
                    </label>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <a
                      className="sc-iHbSHJ bwYSnM"
                      href="/swap?base=.&amp;quote=8db269c3ec630e06ae29f74bc39edd1f87c819f1056206e879a1cd61.446a65644d6963726f555344"
                    >
                      <div
                        className="LazyLoad is-visible"
                        style={{ height: "32px", width: "59.2px" }}
                      >
                        <div
                          width="59.2px"
                          height="32px"
                          className="sc-cWSHoV dazaWi"
                        >
                          <img
                            src="https://tokens.muesliswap.com/static/img/tokens/8db269c3ec630e06ae29f74bc39edd1f87c819f1056206e879a1cd61.446a65644d6963726f555344_scaled_100.webp"
                            crossOrigin="anonymous"
                            alt="https://tokens.muesliswap.com/static/img/tokens/8db269c3ec630e06ae29f74bc39edd1f87c819f1056206e879a1cd61.446a65644d6963726f555344_scaled_100.webp"
                            className="sc-gEvEer ceiwSH"
                          />
                          <img
                            src="/static/media/ada.ae3e320f25e324286ae2.webp"
                            crossOrigin="anonymous"
                            alt=""
                            className="sc-gEvEer gUHNA-d"
                          />
                        </div>
                      </div>
                      <span className="sc-Nxspf ebZgeq">DJED</span>
                      <span className="sc-klVQfs cErbak">Verified</span>
                      <span className="sc-klVQfs bRcUix"> Popular </span>
                      <span className="sc-klVQfs bRcUix"> DeFi </span>
                    </a>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">2.794819</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      0.988$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div value="0" className="sc-uVWWZ eJsMUQ">
                      &nbsp;0%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div value="2.3916483232951258" className="sc-uVWWZ lcUsoT">
                      ↑&nbsp;2.39%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">5,460</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      1,930$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">7,160,083</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      2,530,839$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div className="sc-gEvEer fiARsy">
                      <canvas
                        height="48"
                        width="112"
                        id="markets-graph-cell-8db269c3ec630e06ae29f74bc39edd1f87c819f1056206e879a1cd61.446a65644d6963726f555344"
                        style={{ width: "112px", height: "48px" }}
                      ></canvas>
                    </div>
                  </td>
                </tr>
                <tr role="row" className="sc-gmPhUn FCgni">
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <label className="sc-kAkpmW cRPsKG">
                      <input type="checkbox" className="sc-cmaqmh iwhYbG" />
                      <svg
                        width="20"
                        height="20"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M5.91 18.32a1.68 1.68 0 0 1-1.65-1.95l.65-3.97-2.77-2.86a1.68 1.68 0 0 1 .95-2.82l3.74-.58 1.65-3.53a1.69 1.69 0 0 1 3.03 0l1.65 3.53 3.75.58a1.68 1.68 0 0 1 .95 2.82L15.1 12.4l.64 3.98a1.67 1.67 0 0 1-2.47 1.74L10 16.29l-3.27 1.82a1.67 1.67 0 0 1-.82.21Zm4.09-3.7c.28 0 .56.08.81.22l3.27 1.81-.63-3.98a1.69 1.69 0 0 1 .45-1.44l2.76-2.85-3.75-.6a1.67 1.67 0 0 1-1.26-.94l-1.64-3.52-1.66 3.52a1.67 1.67 0 0 1-1.26.95l-3.75.57 2.76 2.87c.37.38.54.91.45 1.44l-.64 3.97 3.28-1.8c.25-.14.53-.21.81-.22Z"
                          fill="currentColor"
                          fillRule="evenodd"
                        ></path>
                      </svg>
                    </label>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <a
                      className="sc-iHbSHJ bwYSnM"
                      href="/swap?base=.&amp;quote=92776616f1f32c65a173392e4410a3d8c39dcf6ef768c73af164779c.4d79555344"
                    >
                      <div
                        className="LazyLoad is-visible"
                        style={{ height: "32px", width: "59.2px" }}
                      >
                        <div
                          width="59.2px"
                          height="32px"
                          className="sc-cWSHoV dazaWi"
                        >
                          <img
                            src="https://tokens.muesliswap.com/static/img/tokens/92776616f1f32c65a173392e4410a3d8c39dcf6ef768c73af164779c.4d79555344_scaled_100.webp"
                            crossOrigin="anonymous"
                            alt="https://tokens.muesliswap.com/static/img/tokens/92776616f1f32c65a173392e4410a3d8c39dcf6ef768c73af164779c.4d79555344_scaled_100.webp"
                            className="sc-gEvEer ceiwSH"
                          />
                          <img
                            src="/static/media/ada.ae3e320f25e324286ae2.webp"
                            crossOrigin="anonymous"
                            alt=""
                            className="sc-gEvEer gUHNA-d"
                          />
                        </div>
                      </div>
                      <span className="sc-Nxspf ebZgeq">MyUSD</span>
                      <span className="sc-klVQfs cErbak">Verified</span>
                      <span className="sc-klVQfs bRcUix"> DeFi </span>
                    </a>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">-</p>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div value="0" className="sc-uVWWZ eJsMUQ">
                      &nbsp;0%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div value="0" className="sc-uVWWZ eJsMUQ">
                      &nbsp;0%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">4,936</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      1,745$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">-</p>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div className="sc-gEvEer fiARsy">
                      <canvas
                        height="48"
                        width="112"
                        id="markets-graph-cell-92776616f1f32c65a173392e4410a3d8c39dcf6ef768c73af164779c.4d79555344"
                        style={{ width: "112px", height: "48px" }}
                      ></canvas>
                    </div>
                  </td>
                </tr>
                <tr role="row" className="sc-gmPhUn FCgni">
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <label className="sc-kAkpmW cRPsKG">
                      <input type="checkbox" className="sc-cmaqmh iwhYbG" />
                      <svg
                        width="20"
                        height="20"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M5.91 18.32a1.68 1.68 0 0 1-1.65-1.95l.65-3.97-2.77-2.86a1.68 1.68 0 0 1 .95-2.82l3.74-.58 1.65-3.53a1.69 1.69 0 0 1 3.03 0l1.65 3.53 3.75.58a1.68 1.68 0 0 1 .95 2.82L15.1 12.4l.64 3.98a1.67 1.67 0 0 1-2.47 1.74L10 16.29l-3.27 1.82a1.67 1.67 0 0 1-.82.21Zm4.09-3.7c.28 0 .56.08.81.22l3.27 1.81-.63-3.98a1.69 1.69 0 0 1 .45-1.44l2.76-2.85-3.75-.6a1.67 1.67 0 0 1-1.26-.94l-1.64-3.52-1.66 3.52a1.67 1.67 0 0 1-1.26.95l-3.75.57 2.76 2.87c.37.38.54.91.45 1.44l-.64 3.97 3.28-1.8c.25-.14.53-.21.81-.22Z"
                          fill="currentColor"
                          fillRule="evenodd"
                        ></path>
                      </svg>
                    </label>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <a
                      className="sc-iHbSHJ bwYSnM"
                      href="/swap?base=.&amp;quote=682fe60c9918842b3323c43b5144bc3d52a23bd2fb81345560d73f63.4e45574d"
                    >
                      <div
                        className="LazyLoad is-visible"
                        style={{ height: "32px", width: "59.2px" }}
                      >
                        <div
                          width="59.2px"
                          height="32px"
                          className="sc-cWSHoV dazaWi"
                        >
                          <img
                            src="https://tokens.muesliswap.com/static/img/tokens/682fe60c9918842b3323c43b5144bc3d52a23bd2fb81345560d73f63.4e45574d_scaled_100.webp"
                            crossOrigin="anonymous"
                            alt="https://tokens.muesliswap.com/static/img/tokens/682fe60c9918842b3323c43b5144bc3d52a23bd2fb81345560d73f63.4e45574d_scaled_100.webp"
                            className="sc-gEvEer ceiwSH"
                          />
                          <img
                            src="/static/media/ada.ae3e320f25e324286ae2.webp"
                            crossOrigin="anonymous"
                            alt=""
                            className="sc-gEvEer gUHNA-d"
                          />
                        </div>
                      </div>
                      <span className="sc-Nxspf ebZgeq">NEWM</span>
                      <span className="sc-klVQfs cErbak">Verified</span>
                    </a>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">0.006038</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      0.002$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div value="0" className="sc-uVWWZ eJsMUQ">
                      &nbsp;0%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div value="0" className="sc-uVWWZ eJsMUQ">
                      &nbsp;0%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">4,491</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      1,587$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">10,757,070</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      3,802,248$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div className="sc-gEvEer fiARsy">
                      <canvas
                        height="48"
                        width="112"
                        id="markets-graph-cell-682fe60c9918842b3323c43b5144bc3d52a23bd2fb81345560d73f63.4e45574d"
                        style={{ width: "112px", height: "48px" }}
                      ></canvas>
                    </div>
                  </td>
                </tr>
                <tr role="row" className="sc-gmPhUn FCgni">
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <label className="sc-kAkpmW cRPsKG">
                      <input type="checkbox" className="sc-cmaqmh iwhYbG" />
                      <svg
                        width="20"
                        height="20"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M5.91 18.32a1.68 1.68 0 0 1-1.65-1.95l.65-3.97-2.77-2.86a1.68 1.68 0 0 1 .95-2.82l3.74-.58 1.65-3.53a1.69 1.69 0 0 1 3.03 0l1.65 3.53 3.75.58a1.68 1.68 0 0 1 .95 2.82L15.1 12.4l.64 3.98a1.67 1.67 0 0 1-2.47 1.74L10 16.29l-3.27 1.82a1.67 1.67 0 0 1-.82.21Zm4.09-3.7c.28 0 .56.08.81.22l3.27 1.81-.63-3.98a1.69 1.69 0 0 1 .45-1.44l2.76-2.85-3.75-.6a1.67 1.67 0 0 1-1.26-.94l-1.64-3.52-1.66 3.52a1.67 1.67 0 0 1-1.26.95l-3.75.57 2.76 2.87c.37.38.54.91.45 1.44l-.64 3.97 3.28-1.8c.25-.14.53-.21.81-.22Z"
                          fill="currentColor"
                          fillRule="evenodd"
                        ></path>
                      </svg>
                    </label>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <a
                      className="sc-iHbSHJ bwYSnM"
                      href="/swap?base=.&amp;quote=afbe91c0b44b3040e360057bf8354ead8c49c4979ae6ab7c4fbdc9eb.4d494c4b7632"
                    >
                      <div
                        className="LazyLoad is-visible"
                        style={{ height: "32px", width: "59.2px" }}
                      >
                        <div
                          width="59.2px"
                          height="32px"
                          className="sc-cWSHoV dazaWi"
                        >
                          <img
                            src="https://tokens.muesliswap.com/static/img/tokens/afbe91c0b44b3040e360057bf8354ead8c49c4979ae6ab7c4fbdc9eb.4d494c4b7632_scaled_100.webp"
                            crossOrigin="anonymous"
                            alt="https://tokens.muesliswap.com/static/img/tokens/afbe91c0b44b3040e360057bf8354ead8c49c4979ae6ab7c4fbdc9eb.4d494c4b7632_scaled_100.webp"
                            className="sc-gEvEer ceiwSH"
                          />
                          <img
                            src="/static/media/ada.ae3e320f25e324286ae2.webp"
                            crossOrigin="anonymous"
                            alt=""
                            className="sc-gEvEer gUHNA-d"
                          />
                        </div>
                      </div>
                      <span className="sc-Nxspf ebZgeq">MILK</span>
                      <span className="sc-klVQfs cErbak">Verified</span>
                      <span className="sc-klVQfs bRcUix"> Popular </span>
                      <span className="sc-klVQfs bRcUix"> DeFi </span>
                    </a>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">1.544756</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      0.546$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div
                      value="-0.5852823953488295"
                      className="sc-uVWWZ ejrJPX"
                    >
                      ↓&nbsp;0.59%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div value="-6.166633869586258" className="sc-uVWWZ ejrJPX">
                      ↓&nbsp;6.17%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">4,157</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      1,469$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">8,959,586</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      3,166,900$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div className="sc-gEvEer fiARsy">
                      <canvas
                        height="48"
                        width="112"
                        id="markets-graph-cell-afbe91c0b44b3040e360057bf8354ead8c49c4979ae6ab7c4fbdc9eb.4d494c4b7632"
                        style={{ width: "112px", height: "48px" }}
                      ></canvas>
                    </div>
                  </td>
                </tr>
                <tr role="row" className="sc-gmPhUn FCgni">
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <label className="sc-kAkpmW cRPsKG">
                      <input type="checkbox" className="sc-cmaqmh iwhYbG" />
                      <svg
                        width="20"
                        height="20"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M5.91 18.32a1.68 1.68 0 0 1-1.65-1.95l.65-3.97-2.77-2.86a1.68 1.68 0 0 1 .95-2.82l3.74-.58 1.65-3.53a1.69 1.69 0 0 1 3.03 0l1.65 3.53 3.75.58a1.68 1.68 0 0 1 .95 2.82L15.1 12.4l.64 3.98a1.67 1.67 0 0 1-2.47 1.74L10 16.29l-3.27 1.82a1.67 1.67 0 0 1-.82.21Zm4.09-3.7c.28 0 .56.08.81.22l3.27 1.81-.63-3.98a1.69 1.69 0 0 1 .45-1.44l2.76-2.85-3.75-.6a1.67 1.67 0 0 1-1.26-.94l-1.64-3.52-1.66 3.52a1.67 1.67 0 0 1-1.26.95l-3.75.57 2.76 2.87c.37.38.54.91.45 1.44l-.64 3.97 3.28-1.8c.25-.14.53-.21.81-.22Z"
                          fill="currentColor"
                          fillRule="evenodd"
                        ></path>
                      </svg>
                    </label>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <a
                      className="sc-iHbSHJ bwYSnM"
                      href="/swap?base=.&amp;quote=1ddcb9c9de95361565392c5bdff64767492d61a96166cb16094e54be.4f5054"
                    >
                      <div
                        className="LazyLoad is-visible"
                        style={{ height: "32px", width: "59.2px" }}
                      >
                        <div
                          width="59.2px"
                          height="32px"
                          className="sc-cWSHoV dazaWi"
                        >
                          <img
                            src="https://tokens.muesliswap.com/static/img/tokens/1ddcb9c9de95361565392c5bdff64767492d61a96166cb16094e54be.4f5054_scaled_100.webp"
                            crossOrigin="anonymous"
                            alt="https://tokens.muesliswap.com/static/img/tokens/1ddcb9c9de95361565392c5bdff64767492d61a96166cb16094e54be.4f5054_scaled_100.webp"
                            className="sc-gEvEer ceiwSH"
                          />
                          <img
                            src="/static/media/ada.ae3e320f25e324286ae2.webp"
                            crossOrigin="anonymous"
                            alt=""
                            className="sc-gEvEer gUHNA-d"
                          />
                        </div>
                      </div>
                      <span className="sc-Nxspf ebZgeq">OPT</span>
                      <span className="sc-klVQfs cErbak">Verified</span>
                      <span className="sc-klVQfs bRcUix"> DeFi </span>
                    </a>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">0.044383</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      0.016$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div
                      value="0.21116879967724156"
                      className="sc-uVWWZ lcUsoT"
                    >
                      ↑&nbsp;0.21%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div value="11.79300441538096" className="sc-uVWWZ lcUsoT">
                      ↑&nbsp;11.79%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">2,919</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      1,032$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">1,773,880</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      627,005$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div className="sc-gEvEer fiARsy">
                      <canvas
                        height="48"
                        width="112"
                        id="markets-graph-cell-1ddcb9c9de95361565392c5bdff64767492d61a96166cb16094e54be.4f5054"
                        style={{ width: "112px", height: "48px" }}
                      ></canvas>
                    </div>
                  </td>
                </tr>
                <tr role="row" className="sc-gmPhUn FCgni">
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <label className="sc-kAkpmW cRPsKG">
                      <input type="checkbox" className="sc-cmaqmh iwhYbG" />
                      <svg
                        width="20"
                        height="20"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M5.91 18.32a1.68 1.68 0 0 1-1.65-1.95l.65-3.97-2.77-2.86a1.68 1.68 0 0 1 .95-2.82l3.74-.58 1.65-3.53a1.69 1.69 0 0 1 3.03 0l1.65 3.53 3.75.58a1.68 1.68 0 0 1 .95 2.82L15.1 12.4l.64 3.98a1.67 1.67 0 0 1-2.47 1.74L10 16.29l-3.27 1.82a1.67 1.67 0 0 1-.82.21Zm4.09-3.7c.28 0 .56.08.81.22l3.27 1.81-.63-3.98a1.69 1.69 0 0 1 .45-1.44l2.76-2.85-3.75-.6a1.67 1.67 0 0 1-1.26-.94l-1.64-3.52-1.66 3.52a1.67 1.67 0 0 1-1.26.95l-3.75.57 2.76 2.87c.37.38.54.91.45 1.44l-.64 3.97 3.28-1.8c.25-.14.53-.21.81-.22Z"
                          fill="currentColor"
                          fillRule="evenodd"
                        ></path>
                      </svg>
                    </label>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <a
                      className="sc-iHbSHJ bwYSnM"
                      href="/swap?base=.&amp;quote=7914fae20eb2903ed6fd5021a415c1bd2626b64a2d86a304cb40ff5e.4c494649"
                    >
                      <div
                        className="LazyLoad"
                        style={{ height: "32px", width: "59.2px" }}
                      ></div>
                      <span className="sc-Nxspf ebZgeq">LIFI</span>
                      <span className="sc-klVQfs cErbak">Verified</span>
                      <span className="sc-klVQfs bRcUix"> Popular </span>
                      <span className="sc-klVQfs bRcUix"> DeFi </span>
                    </a>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">0.062026</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      0.022$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div
                      value="-0.2699915748332282"
                      className="sc-uVWWZ ejrJPX"
                    >
                      ↓&nbsp;0.27%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div value="-3.729107040081081" className="sc-uVWWZ ejrJPX">
                      ↓&nbsp;3.73%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">2,734</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      966$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">1,850,807</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      654,195$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div className="sc-gEvEer fiARsy">
                      <canvas
                        height="48"
                        width="112"
                        id="markets-graph-cell-7914fae20eb2903ed6fd5021a415c1bd2626b64a2d86a304cb40ff5e.4c494649"
                        style={{ width: "112px", height: "48px" }}
                      ></canvas>
                    </div>
                  </td>
                </tr>
                <tr role="row" className="sc-gmPhUn FCgni">
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <label className="sc-kAkpmW cRPsKG">
                      <input type="checkbox" className="sc-cmaqmh iwhYbG" />
                      <svg
                        width="20"
                        height="20"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M5.91 18.32a1.68 1.68 0 0 1-1.65-1.95l.65-3.97-2.77-2.86a1.68 1.68 0 0 1 .95-2.82l3.74-.58 1.65-3.53a1.69 1.69 0 0 1 3.03 0l1.65 3.53 3.75.58a1.68 1.68 0 0 1 .95 2.82L15.1 12.4l.64 3.98a1.67 1.67 0 0 1-2.47 1.74L10 16.29l-3.27 1.82a1.67 1.67 0 0 1-.82.21Zm4.09-3.7c.28 0 .56.08.81.22l3.27 1.81-.63-3.98a1.69 1.69 0 0 1 .45-1.44l2.76-2.85-3.75-.6a1.67 1.67 0 0 1-1.26-.94l-1.64-3.52-1.66 3.52a1.67 1.67 0 0 1-1.26.95l-3.75.57 2.76 2.87c.37.38.54.91.45 1.44l-.64 3.97 3.28-1.8c.25-.14.53-.21.81-.22Z"
                          fill="currentColor"
                          fillRule="evenodd"
                        ></path>
                      </svg>
                    </label>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <a
                      className="sc-iHbSHJ bwYSnM"
                      href="/swap?base=.&amp;quote=577f0b1342f8f8f4aed3388b80a8535812950c7a892495c0ecdf0f1e.0014df10464c4454"
                    >
                      <div
                        className="LazyLoad"
                        style={{ height: "32px", width: "59.2px" }}
                      ></div>
                      <span className="sc-Nxspf ebZgeq">FLDT</span>
                      <span className="sc-klVQfs cErbak">Verified</span>
                    </a>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">0.198629</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      0.07$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div value="0" className="sc-uVWWZ eJsMUQ">
                      &nbsp;0%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div value="0" className="sc-uVWWZ eJsMUQ">
                      &nbsp;0%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">1,870</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      661$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">6,560,775</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      2,319,004$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div className="sc-gEvEer fiARsy">
                      <canvas
                        height="48"
                        width="112"
                        id="markets-graph-cell-577f0b1342f8f8f4aed3388b80a8535812950c7a892495c0ecdf0f1e.0014df10464c4454"
                        style={{ width: "112px", height: "48px" }}
                      ></canvas>
                    </div>
                  </td>
                </tr>
                <tr role="row" className="sc-gmPhUn FCgni">
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <label className="sc-kAkpmW cRPsKG">
                      <input type="checkbox" className="sc-cmaqmh iwhYbG" />
                      <svg
                        width="20"
                        height="20"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M5.91 18.32a1.68 1.68 0 0 1-1.65-1.95l.65-3.97-2.77-2.86a1.68 1.68 0 0 1 .95-2.82l3.74-.58 1.65-3.53a1.69 1.69 0 0 1 3.03 0l1.65 3.53 3.75.58a1.68 1.68 0 0 1 .95 2.82L15.1 12.4l.64 3.98a1.67 1.67 0 0 1-2.47 1.74L10 16.29l-3.27 1.82a1.67 1.67 0 0 1-.82.21Zm4.09-3.7c.28 0 .56.08.81.22l3.27 1.81-.63-3.98a1.69 1.69 0 0 1 .45-1.44l2.76-2.85-3.75-.6a1.67 1.67 0 0 1-1.26-.94l-1.64-3.52-1.66 3.52a1.67 1.67 0 0 1-1.26.95l-3.75.57 2.76 2.87c.37.38.54.91.45 1.44l-.64 3.97 3.28-1.8c.25-.14.53-.21.81-.22Z"
                          fill="currentColor"
                          fillRule="evenodd"
                        ></path>
                      </svg>
                    </label>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <a
                      className="sc-iHbSHJ bwYSnM"
                      href="/swap?base=.&amp;quote=a0028f350aaabe0545fdcb56b039bfb08e4bb4d8c4d7c3c7d481c235.484f534b59"
                    >
                      <div
                        className="LazyLoad"
                        style={{ height: "32px", width: "59.2px" }}
                      ></div>
                      <span className="sc-Nxspf ebZgeq">HOSKY</span>
                      <span className="sc-klVQfs cErbak">Verified</span>
                      <span className="sc-klVQfs bRcUix"> Popular </span>
                      <span className="sc-klVQfs bRcUix"> Meme </span>
                    </a>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">2.801E-8</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      9.902E-9$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div
                      value="-1.2692768522545528"
                      className="sc-uVWWZ ejrJPX"
                    >
                      ↓&nbsp;1.27%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div
                      value="-0.5075510208888079"
                      className="sc-uVWWZ ejrJPX"
                    >
                      ↓&nbsp;0.51%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">1,854</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      655$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">28,012,966</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      9,901,603$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div className="sc-gEvEer fiARsy">
                      <canvas
                        height="48"
                        width="112"
                        id="markets-graph-cell-a0028f350aaabe0545fdcb56b039bfb08e4bb4d8c4d7c3c7d481c235.484f534b59"
                        style={{ width: "112px", height: "48px" }}
                      ></canvas>
                    </div>
                  </td>
                </tr>
                <tr role="row" className="sc-gmPhUn FCgni">
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <label className="sc-kAkpmW cRPsKG">
                      <input type="checkbox" className="sc-cmaqmh iwhYbG" />
                      <svg
                        width="20"
                        height="20"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M5.91 18.32a1.68 1.68 0 0 1-1.65-1.95l.65-3.97-2.77-2.86a1.68 1.68 0 0 1 .95-2.82l3.74-.58 1.65-3.53a1.69 1.69 0 0 1 3.03 0l1.65 3.53 3.75.58a1.68 1.68 0 0 1 .95 2.82L15.1 12.4l.64 3.98a1.67 1.67 0 0 1-2.47 1.74L10 16.29l-3.27 1.82a1.67 1.67 0 0 1-.82.21Zm4.09-3.7c.28 0 .56.08.81.22l3.27 1.81-.63-3.98a1.69 1.69 0 0 1 .45-1.44l2.76-2.85-3.75-.6a1.67 1.67 0 0 1-1.26-.94l-1.64-3.52-1.66 3.52a1.67 1.67 0 0 1-1.26.95l-3.75.57 2.76 2.87c.37.38.54.91.45 1.44l-.64 3.97 3.28-1.8c.25-.14.53-.21.81-.22Z"
                          fill="currentColor"
                          fillRule="evenodd"
                        ></path>
                      </svg>
                    </label>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <a
                      className="sc-iHbSHJ bwYSnM"
                      href="/swap?base=.&amp;quote=b6a7467ea1deb012808ef4e87b5ff371e85f7142d7b356a40d9b42a0.436f726e75636f70696173205b76696120436861696e506f72742e696f5d"
                    >
                      <div
                        className="LazyLoad"
                        style={{ height: "32px", width: "59.2px" }}
                      ></div>
                      <span className="sc-Nxspf ebZgeq">COPI</span>
                      <span className="sc-klVQfs cErbak">Verified</span>
                      <span className="sc-klVQfs bRcUix"> Other </span>
                    </a>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">0.070054</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      0.025$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div value="0" className="sc-uVWWZ eJsMUQ">
                      &nbsp;0%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div value="0" className="sc-uVWWZ eJsMUQ">
                      &nbsp;0%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">1,581</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      559$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">64,092,344</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      22,654,400$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div className="sc-gEvEer fiARsy">
                      <canvas
                        height="48"
                        width="112"
                        id="markets-graph-cell-b6a7467ea1deb012808ef4e87b5ff371e85f7142d7b356a40d9b42a0.436f726e75636f70696173205b76696120436861696e506f72742e696f5d"
                        style={{ width: "112px", height: "48px" }}
                      ></canvas>
                    </div>
                  </td>
                </tr>
                <tr role="row" className="sc-gmPhUn FCgni">
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <label className="sc-kAkpmW cRPsKG">
                      <input type="checkbox" className="sc-cmaqmh iwhYbG" />
                      <svg
                        width="20"
                        height="20"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M5.91 18.32a1.68 1.68 0 0 1-1.65-1.95l.65-3.97-2.77-2.86a1.68 1.68 0 0 1 .95-2.82l3.74-.58 1.65-3.53a1.69 1.69 0 0 1 3.03 0l1.65 3.53 3.75.58a1.68 1.68 0 0 1 .95 2.82L15.1 12.4l.64 3.98a1.67 1.67 0 0 1-2.47 1.74L10 16.29l-3.27 1.82a1.67 1.67 0 0 1-.82.21Zm4.09-3.7c.28 0 .56.08.81.22l3.27 1.81-.63-3.98a1.69 1.69 0 0 1 .45-1.44l2.76-2.85-3.75-.6a1.67 1.67 0 0 1-1.26-.94l-1.64-3.52-1.66 3.52a1.67 1.67 0 0 1-1.26.95l-3.75.57 2.76 2.87c.37.38.54.91.45 1.44l-.64 3.97 3.28-1.8c.25-.14.53-.21.81-.22Z"
                          fill="currentColor"
                          fillRule="evenodd"
                        ></path>
                      </svg>
                    </label>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <a
                      className="sc-iHbSHJ bwYSnM"
                      href="/swap?base=.&amp;quote=edfd7a1d77bcb8b884c474bdc92a16002d1fb720e454fa6e99344479.4e5458"
                    >
                      <div
                        className="LazyLoad"
                        style={{ height: "32px", width: "59.2px" }}
                      ></div>
                      <span className="sc-Nxspf ebZgeq">NTX</span>
                      <span className="sc-klVQfs cErbak">Verified</span>
                    </a>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">0.09295</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      0.033$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div value="1.2097806073788409" className="sc-uVWWZ lcUsoT">
                      ↑&nbsp;1.21%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div value="5.423489120883174" className="sc-uVWWZ lcUsoT">
                      ↑&nbsp;5.42%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">1,300</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      460$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">61,780,269</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      21,837,163$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div className="sc-gEvEer fiARsy">
                      <canvas
                        height="48"
                        width="112"
                        id="markets-graph-cell-edfd7a1d77bcb8b884c474bdc92a16002d1fb720e454fa6e99344479.4e5458"
                        style={{ width: "112px", height: "48px" }}
                      ></canvas>
                    </div>
                  </td>
                </tr>
                <tr role="row" className="sc-gmPhUn FCgni">
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <label className="sc-kAkpmW cRPsKG">
                      <input type="checkbox" className="sc-cmaqmh iwhYbG" />
                      <svg
                        width="20"
                        height="20"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M5.91 18.32a1.68 1.68 0 0 1-1.65-1.95l.65-3.97-2.77-2.86a1.68 1.68 0 0 1 .95-2.82l3.74-.58 1.65-3.53a1.69 1.69 0 0 1 3.03 0l1.65 3.53 3.75.58a1.68 1.68 0 0 1 .95 2.82L15.1 12.4l.64 3.98a1.67 1.67 0 0 1-2.47 1.74L10 16.29l-3.27 1.82a1.67 1.67 0 0 1-.82.21Zm4.09-3.7c.28 0 .56.08.81.22l3.27 1.81-.63-3.98a1.69 1.69 0 0 1 .45-1.44l2.76-2.85-3.75-.6a1.67 1.67 0 0 1-1.26-.94l-1.64-3.52-1.66 3.52a1.67 1.67 0 0 1-1.26.95l-3.75.57 2.76 2.87c.37.38.54.91.45 1.44l-.64 3.97 3.28-1.8c.25-.14.53-.21.81-.22Z"
                          fill="currentColor"
                          fillRule="evenodd"
                        ></path>
                      </svg>
                    </label>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <a
                      className="sc-iHbSHJ bwYSnM"
                      href="/swap?base=.&amp;quote=7507734918533b3b896241b4704f3d4ce805256b01da6fcede430436.42616279534e454b"
                    >
                      <div
                        className="LazyLoad"
                        style={{ height: "32px", width: "59.2px" }}
                      ></div>
                      <span className="sc-Nxspf ebZgeq">BabySNEK</span>
                      <span className="sc-klVQfs cErbak">Verified</span>
                      <span className="sc-klVQfs bRcUix"> Meme </span>
                    </a>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">0.000008</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      2.755E-6$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div value="0" className="sc-uVWWZ eJsMUQ">
                      &nbsp;0%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div value="0" className="sc-uVWWZ eJsMUQ">
                      &nbsp;0%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">531</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      188$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">583,983</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      206,418$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div className="sc-gEvEer fiARsy">
                      <canvas
                        height="48"
                        width="112"
                        id="markets-graph-cell-7507734918533b3b896241b4704f3d4ce805256b01da6fcede430436.42616279534e454b"
                        style={{ width: "112px", height: "48px" }}
                      ></canvas>
                    </div>
                  </td>
                </tr>
                <tr role="row" className="sc-gmPhUn FCgni">
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <label className="sc-kAkpmW cRPsKG">
                      <input type="checkbox" className="sc-cmaqmh iwhYbG" />
                      <svg
                        width="20"
                        height="20"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M5.91 18.32a1.68 1.68 0 0 1-1.65-1.95l.65-3.97-2.77-2.86a1.68 1.68 0 0 1 .95-2.82l3.74-.58 1.65-3.53a1.69 1.69 0 0 1 3.03 0l1.65 3.53 3.75.58a1.68 1.68 0 0 1 .95 2.82L15.1 12.4l.64 3.98a1.67 1.67 0 0 1-2.47 1.74L10 16.29l-3.27 1.82a1.67 1.67 0 0 1-.82.21Zm4.09-3.7c.28 0 .56.08.81.22l3.27 1.81-.63-3.98a1.69 1.69 0 0 1 .45-1.44l2.76-2.85-3.75-.6a1.67 1.67 0 0 1-1.26-.94l-1.64-3.52-1.66 3.52a1.67 1.67 0 0 1-1.26.95l-3.75.57 2.76 2.87c.37.38.54.91.45 1.44l-.64 3.97 3.28-1.8c.25-.14.53-.21.81-.22Z"
                          fill="currentColor"
                          fillRule="evenodd"
                        ></path>
                      </svg>
                    </label>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <a
                      className="sc-iHbSHJ bwYSnM"
                      href="/swap?base=.&amp;quote=a2944573e99d2ed3055b808eaa264f0bf119e01fc6b18863067c63e4.4d454c44"
                    >
                      <div
                        className="LazyLoad"
                        style={{ height: "32px", width: "59.2px" }}
                      ></div>
                      <span className="sc-Nxspf ebZgeq">MELD</span>
                      <span className="sc-klVQfs cErbak">Verified</span>
                    </a>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">0.036328</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      0.013$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div value="0" className="sc-uVWWZ eJsMUQ">
                      &nbsp;0%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div value="2.3218683704619103" className="sc-uVWWZ lcUsoT">
                      ↑&nbsp;2.32%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">440</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      156$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">44,663,193</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      15,786,876$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div className="sc-gEvEer fiARsy">
                      <canvas
                        height="48"
                        width="112"
                        id="markets-graph-cell-a2944573e99d2ed3055b808eaa264f0bf119e01fc6b18863067c63e4.4d454c44"
                        style={{ width: "112px", height: "48px" }}
                      ></canvas>
                    </div>
                  </td>
                </tr>
                <tr role="row" className="sc-gmPhUn FCgni">
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <label className="sc-kAkpmW cRPsKG">
                      <input type="checkbox" className="sc-cmaqmh iwhYbG" />
                      <svg
                        width="20"
                        height="20"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M5.91 18.32a1.68 1.68 0 0 1-1.65-1.95l.65-3.97-2.77-2.86a1.68 1.68 0 0 1 .95-2.82l3.74-.58 1.65-3.53a1.69 1.69 0 0 1 3.03 0l1.65 3.53 3.75.58a1.68 1.68 0 0 1 .95 2.82L15.1 12.4l.64 3.98a1.67 1.67 0 0 1-2.47 1.74L10 16.29l-3.27 1.82a1.67 1.67 0 0 1-.82.21Zm4.09-3.7c.28 0 .56.08.81.22l3.27 1.81-.63-3.98a1.69 1.69 0 0 1 .45-1.44l2.76-2.85-3.75-.6a1.67 1.67 0 0 1-1.26-.94l-1.64-3.52-1.66 3.52a1.67 1.67 0 0 1-1.26.95l-3.75.57 2.76 2.87c.37.38.54.91.45 1.44l-.64 3.97 3.28-1.8c.25-.14.53-.21.81-.22Z"
                          fill="currentColor"
                          fillRule="evenodd"
                        ></path>
                      </svg>
                    </label>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <a
                      className="sc-iHbSHJ bwYSnM"
                      href="/swap?base=.&amp;quote=6ac8ef33b510ec004fe11585f7c5a9f0c07f0c23428ab4f29c1d7d10.4d454c44"
                    >
                      <div
                        className="LazyLoad"
                        style={{ height: "32px", width: "59.2px" }}
                      ></div>
                      <span className="sc-Nxspf ebZgeq">MELD</span>
                      <span className="sc-klVQfs cErbak">Verified</span>
                    </a>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">0.036228</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      0.013$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div value="0" className="sc-uVWWZ eJsMUQ">
                      &nbsp;0%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div value="1.9420884772600955" className="sc-uVWWZ lcUsoT">
                      ↑&nbsp;1.94%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">440</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      156$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">144,910,947</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      51,220,948$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div className="sc-gEvEer fiARsy">
                      <canvas
                        height="48"
                        width="112"
                        id="markets-graph-cell-6ac8ef33b510ec004fe11585f7c5a9f0c07f0c23428ab4f29c1d7d10.4d454c44"
                        style={{ width: "112px", height: "48px" }}
                      ></canvas>
                    </div>
                  </td>
                </tr>
                <tr role="row" className="sc-gmPhUn FCgni">
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <label className="sc-kAkpmW cRPsKG">
                      <input type="checkbox" className="sc-cmaqmh iwhYbG" />
                      <svg
                        width="20"
                        height="20"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M5.91 18.32a1.68 1.68 0 0 1-1.65-1.95l.65-3.97-2.77-2.86a1.68 1.68 0 0 1 .95-2.82l3.74-.58 1.65-3.53a1.69 1.69 0 0 1 3.03 0l1.65 3.53 3.75.58a1.68 1.68 0 0 1 .95 2.82L15.1 12.4l.64 3.98a1.67 1.67 0 0 1-2.47 1.74L10 16.29l-3.27 1.82a1.67 1.67 0 0 1-.82.21Zm4.09-3.7c.28 0 .56.08.81.22l3.27 1.81-.63-3.98a1.69 1.69 0 0 1 .45-1.44l2.76-2.85-3.75-.6a1.67 1.67 0 0 1-1.26-.94l-1.64-3.52-1.66 3.52a1.67 1.67 0 0 1-1.26.95l-3.75.57 2.76 2.87c.37.38.54.91.45 1.44l-.64 3.97 3.28-1.8c.25-.14.53-.21.81-.22Z"
                          fill="currentColor"
                          fillRule="evenodd"
                        ></path>
                      </svg>
                    </label>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <a
                      className="sc-iHbSHJ bwYSnM"
                      href="/swap?base=.&amp;quote=dda5fdb1002f7389b33e036b6afee82a8189becb6cba852e8b79b4fb.0014df1047454e53"
                    >
                      <div
                        className="LazyLoad"
                        style={{ height: "32px", width: "59.2px" }}
                      ></div>
                      <span className="sc-Nxspf ebZgeq">GENS</span>
                      <span className="sc-klVQfs cErbak">Verified</span>
                      <span className="sc-klVQfs bRcUix"> DeFi </span>
                    </a>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">-</p>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div value="0" className="sc-uVWWZ eJsMUQ">
                      &nbsp;0%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div
                      value="-15.536807824020771"
                      className="sc-uVWWZ ejrJPX"
                    >
                      ↓&nbsp;15.54%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">401</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      142$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">-</p>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div className="sc-gEvEer fiARsy">
                      <canvas
                        height="48"
                        width="112"
                        id="markets-graph-cell-dda5fdb1002f7389b33e036b6afee82a8189becb6cba852e8b79b4fb.0014df1047454e53"
                        style={{ width: "112px", height: "48px" }}
                      ></canvas>
                    </div>
                  </td>
                </tr>
                <tr role="row" className="sc-gmPhUn FCgni">
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <label className="sc-kAkpmW cRPsKG">
                      <input type="checkbox" className="sc-cmaqmh iwhYbG" />
                      <svg
                        width="20"
                        height="20"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M5.91 18.32a1.68 1.68 0 0 1-1.65-1.95l.65-3.97-2.77-2.86a1.68 1.68 0 0 1 .95-2.82l3.74-.58 1.65-3.53a1.69 1.69 0 0 1 3.03 0l1.65 3.53 3.75.58a1.68 1.68 0 0 1 .95 2.82L15.1 12.4l.64 3.98a1.67 1.67 0 0 1-2.47 1.74L10 16.29l-3.27 1.82a1.67 1.67 0 0 1-.82.21Zm4.09-3.7c.28 0 .56.08.81.22l3.27 1.81-.63-3.98a1.69 1.69 0 0 1 .45-1.44l2.76-2.85-3.75-.6a1.67 1.67 0 0 1-1.26-.94l-1.64-3.52-1.66 3.52a1.67 1.67 0 0 1-1.26.95l-3.75.57 2.76 2.87c.37.38.54.91.45 1.44l-.64 3.97 3.28-1.8c.25-.14.53-.21.81-.22Z"
                          fill="currentColor"
                          fillRule="evenodd"
                        ></path>
                      </svg>
                    </label>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <a
                      className="sc-iHbSHJ bwYSnM"
                      href="/swap?base=.&amp;quote=51a5e236c4de3af2b8020442e2a26f454fda3b04cb621c1294a0ef34.424f4f4b"
                    >
                      <div
                        className="LazyLoad"
                        style={{ height: "32px", width: "59.2px" }}
                      ></div>
                      <span className="sc-Nxspf ebZgeq">STUFF</span>
                      <span className="sc-klVQfs cErbak">Verified</span>
                    </a>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">0.031044</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      0.011$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div value="0" className="sc-uVWWZ eJsMUQ">
                      &nbsp;0%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div value="0" className="sc-uVWWZ eJsMUQ">
                      &nbsp;0%
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">385</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      136$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <p className="sc-gEvEer dSOvFp">45,393,187</p>
                    <div color="grey500" className="sc-gEvEer bdDaQC">
                      16,044,903$
                    </div>
                  </td>
                  <td role="cell" className="sc-hRJfrW iUJzLW">
                    <div className="sc-gEvEer fiARsy">
                      <canvas
                        height="48"
                        width="112"
                        id="markets-graph-cell-51a5e236c4de3af2b8020442e2a26f454fda3b04cb621c1294a0ef34.424f4f4b"
                        style={{ width: "112px", height: "48px" }}
                      ></canvas>
                    </div>
                  </td>
                </tr>
              </tbody>
            </table>
            <div title="Page Navigation" className="sc-ggpjZQ iWGVXG">
              <label className="sc-gEvEer sc-eqUAAy ejbbHR fgprtA">
                <label width="max-content" className="sc-gEvEer ZpyKi">
                  Show Rows
                </label>
                <div className="sc-gEvEer eLiPBP">
                  <div className="sc-gEvEer sc-eqUAAy iqGkHt fgprtA">
                    <select className="sc-dZoequ kBoiaJ">
                      <option>10</option>
                      <option>20</option>
                      <option>50</option>
                      <option>100</option>
                    </select>
                    <div className="sc-jnOGJG fKMeQu">
                      <svg
                        width="10"
                        height="6"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M.76 6h8.48a.76.76 0 0 0 .7-.5.81.81 0 0 0-.17-.85L5.54.23A.74.74 0 0 0 5 0a.74.74 0 0 0-.54.23L.22 4.65a.81.81 0 0 0-.16.86.76.76 0 0 0 .7.49Z"
                          fill="currentColor"
                          fillRule="evenodd"
                        ></path>
                      </svg>
                    </div>
                  </div>
                </div>
              </label>
              <div width="100%" className="sc-gEvEer sc-eqUAAy beaxRO fgprtA">
                <button
                  type="button"
                  aria-label="previous page"
                  disabled=""
                  value="-1"
                  className="sc-hIUJlX CGLGu"
                >
                  <svg viewBox="0 0 14 12" xmlns="http://www.w3.org/2000/svg">
                    <path
                      d="M7.71 11.72a1 1 0 0 1 0-1.42L11 7H1a1 1 0 0 1 0-2h10L7.71 1.71A1 1 0 1 1 9.12.3l4.3 4.3a2 2 0 0 1 .57 1.24L14 6a2 2 0 0 1-.59 1.4l-4.29 4.3a1 1 0 0 1-1.41.02z"
                      fill="currentColor"
                      fillRule="evenodd"
                    ></path>
                  </svg>
                </button>
                <iframe
                  style={{
                    background: "transparent",
                    border: "none",
                    height: "100%",
                    left: 0,
                    position: "absolute",
                    top: 0,
                    width: "100%",
                    zIndex: -1,
                  }}
                ></iframe>
                <button type="button" value="0" className="sc-fxwrCY lcFNFd">
                  1
                </button>
                <button type="button" value="1" className="sc-fxwrCY bqoKix">
                  2
                </button>
                <button type="button" value="2" className="sc-fxwrCY bqoKix">
                  3
                </button>
                <div className="sc-dSCufp hSZqBO">
                  <div className="sc-gEvEer jSnzCT">...</div>
                </div>
                <button type="button" value="17" className="sc-fxwrCY bqoKix">
                  18
                </button>
                <button type="button" value="18" className="sc-fxwrCY bqoKix">
                  19
                </button>
                <button type="button" value="19" className="sc-fxwrCY bqoKix">
                  20
                </button>
                <button
                  type="button"
                  aria-label="next page"
                  value="1"
                  className="sc-hIUJlX hAswTQ"
                >
                  <svg viewBox="0 0 14 12" xmlns="http://www.w3.org/2000/svg">
                    <path
                      d="M7.71 11.72a1 1 0 0 1 0-1.42L11 7H1a1 1 0 0 1 0-2h10L7.71 1.71A1 1 0 1 1 9.12.3l4.3 4.3a2 2 0 0 1 .57 1.24L14 6a2 2 0 0 1-.59 1.4l-4.29 4.3a1 1 0 0 1-1.41.02z"
                      fill="currentColor"
                      fillRule="evenodd"
                    ></path>
                  </svg>
                </button>
              </div>
            </div>
          </div>
        </div>
      </main>

      <div className="sc-gEvEer sc-eqUAAy fSNpul fgprtA"></div>
    </div>
  );
};

export default Markets;
